...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.indexOf(currentSymbol) === str.lastIndexOf(currentSymbol)) {
      uniqSymbol = currentSymbol;
      break;
    }
  }
  if (uniqSymbol.length === 0) {
    return null;
  }
  return uniqSymbol;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const str = `${openedBracket + Math.min(a, b)}, ${Math.max(a, b) + closedBracket}`;
  return str;
}
...
function getDigitalRoot(num) {
  ...
  const result = numbers.reduce((total, amount) => total + +amount, 0);
